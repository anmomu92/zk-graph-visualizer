...
async function loadAll() {
  ...
  try {
    ...
    if (!graphRes.ok)
      throw new Error(`graph.json: ${graphRes.status} ${graphRes.statusText}`);
    if (!notesRes.ok)
      throw new Error(`notes.json: ${notesRes.status} ${notesRes.statusText}`);

    const [graphRaw, notesRaw] = await Promise.all([
      graphRes.json(),
      notesRes.json(),
    ]);

    const links = Array.isArray(graphRaw)
      ? graphRaw
      : graphRaw.links || graphRaw.edges || graphRaw.data || [];
    const notes = Array.isArray(notesRaw)
      ? notesRaw
      : notesRaw.notes || notesRaw.data || [];
    ...
    document.getElementById("empty-hint").querySelector("h2").textContent =
      "Failed to load";
...
function renderGraph() {
  ...
  nodeSel
    ...
    .attr("r", (d) => nodeR(d))
    .attr("fill", (d) =>
      d.degree >= maxDeg * 0.6 ? "var(--node-hub)" : "var(--node-default)",
    )
    .attr("stroke", (d) =>
      d.degree >= maxDeg * 0.6
        ? cssVar("--node-hub-stroke")
        : cssVar("--node-default-stroke"),
    )
    .style("filter", (d) =>
      d.degree >= maxDeg * 0.6
  ...
  simulation = d3
    ...
    .force(
      "collision",
      d3.forceCollide().radius((d) => nodeR(d) + 10),
    )
    .on("tick", () => {
      linkSel
...
function initControls() {
  document.getElementById("zoom-in").onclick = () => {
    if (zoomBehavior) svg.transition().call(zoomBehavior.scaleBy, 1.4);
  };
  document.getElementById("zoom-out").onclick = () => {
    if (zoomBehavior) svg.transition().call(zoomBehavior.scaleBy, 0.7);
  };
...
function showPanel(d) {
  ...
  if (d.rawContent) {
    ...
    if (window.MathJax && MathJax.typesetPromise) {
      MathJax.typesetPromise([cpane]).catch((err) =>
  ...
  lh +=
    srow("Total links", d.degree) +
    srow("Outgoing", outLinks.length) +
    srow("Incoming", inLinks.length);
  if (outLinks.length) {
  ...
  if (d.tags && d.tags.length) {
    mh +=
      `<div class="section-label mt">Tags</div>
      <div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:4px">` +
      d.tags
        .map(
          (t) =>
            `<span style="background:rgba(61,127,255,0.12);color:var(--accent);border:1px solid rgba(61,127,255,0.25);border-radius:4px;padding:2px 8px;font-size:10px">${esc(t)}</span>`,
        )
        .join("") +
      `</div>`;
  }
...
function renderMd(raw) {
  ...
  function stashLatex(src) {
    return src
      .replace(/\$\$([\s\S]+?)\$\$/g, (_, m) => {
        latex.push(`$$${m}$$`);
        return `\x00L${latex.length - 1}\x00`;
      })
      .replace(/\\\[([\s\S]+?)\\\]/g, (_, m) => {
        latex.push(`\\[${m}\\]`);
        return `\x00L${latex.length - 1}\x00`;
  ...
  s = s.replace(/\[\[([^\]]+)\]\]/g, (_, stem) => {
    ...
    const label =
      node && node.label && node.label !== node.stem ? node.label : stem;
  ...
  s = s.replace(
    /^(?!<[houtbpdr]|<pre|<hr|<blockquote|<div)(.+)$/gm,
    "<p>$1</p>",
  );
  s = s.replace(/<p>\s*<\/p>/g, "");
...
window.addEventListener("resize", () => {
  if (simulation) {
    ...
    simulation.force(
      "center",
      d3.forceCenter(c.clientWidth / 2, c.clientHeight / 2),
    );
